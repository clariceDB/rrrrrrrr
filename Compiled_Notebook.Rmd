...
We provide a third example of the missing functions in the tidygraph, where we are forced to convert the tbl_graph into an igraph. Once again, we need to import the correct packages.
...
As previously mentioned, `triad_census` counts for each triple of vertices, the number of times each of the 16 possible states occur. The states are represented by codes. Some examples can be found below: 
...
Remember that you can call `help(triad_census)` if you want to see a full description of the codes for the states. Before we dive right in, lets take a look at a few smaller graphs.

> <b>Smaller graphs</b>
In order to explain the states, here is a selection of smaller graphs and the results of running `triad_census` on them. We first substantiate a graph and then run `triad_census` on it. 
