...
> <b>Smaller graphs</b>
In order to explain the states, here is a selection of smaller graphs and the results of running `triad_census` on them. We first substantiate a graph (nodes A, B, and C) and then run `triad_census` on it. 
...
Just like the other sections, we initiate our `enron` dataset and create a graph from it. The vertices within the graph is named. 
```{r message=FALSE}
enron <- read_delim("dataraw/email-enron-only.mtx",delim = " ",col_names = F) %>% 
  rename("from" = X1,
         "to" = X2) %>% 
